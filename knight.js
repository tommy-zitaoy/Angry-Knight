class Knight extends Agent {
	...
	loadAnimations() {
		...
		this.animations[3][0] = new Animator(
			this.spritesheet_attack, 0, 0, 128, 64, 15, 0.07, 0, false, false, true);
		this.animations[3][1] = new Animator(
			this.spritesheet_attack, 0, 0, 128, 64, 15, 0.07, 0, false, false, false);
	}
	...
	update() {
		...
		if (this.game.right && this.state != 3) {
			...
			this.vel.x = 0;
			if (this.state != 2 && (this.state == 3 && this.attackCoolDown <= 0)) {
				this.state = 0;
		...
		if (this.game.A && this.attackCoolDown <= 0) {
			if (this.state == 2) this.vel.y = 200;
			this.state = 3;
			this.attackCoolDown = 1.05;
			this.game.A = false;
			this.vel.x = 0;
			// reload attack animation (looping animation causes glitching)
			this.animations[3][0] = new Animator(
				this.spritesheet_attack, 0, 0, 128, 64, 15, 0.07, 0, false, false, true);
			this.animations[3][1] = new Animator(
				this.spritesheet_attack, 0, 0, 128, 64, 15, 0.07, 0, false, false, false);
...
class MeleeAttack extends Agent{
	...
	update() {
		...
		this.pos.y = this.game.knight.pos.y - PARAMS.TILE_WIDTH;
